...
class DuolingoTheme {
  // Duolingo Color Palette (Enhanced)
...
// --- Animated Popup Style ---
class AnimatedPopup extends StatelessWidget {
  final Widget child;
  final Color? backgroundColor;
  const AnimatedPopup({Key? key, required this.child, this.backgroundColor}) : super(key: key);
  @override
...
// --- Pigeon Avatar Widget ---
class PigeonAvatar extends StatelessWidget {
  final double size;
  const PigeonAvatar({Key? key, this.size = 64}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return CircleAvatar(
      radius: size / 2,
      ...
      child: Image.asset(
        'assets/images/pigeon_avatar.png',
        width: size * 0.9,
        height: size * 0.9,
        fit: BoxFit.contain,
      ),
